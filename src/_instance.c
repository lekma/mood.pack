...
static inline int
__object_update_dict__(PyObject *self, PyObject *arg)
{
    ...
    if ((dict = _PyObject_GetAttrId(self, &PyId___dict__))) {
        while ((res = PyDict_Next(arg, &pos, &key, &value))) {
            ...
            /* __dict__ can be a dictionary or other mapping object
               https://docs.python.org/3.10/library/stdtypes.html#object.__dict__ */
...
static inline int
__object_setstate__(PyObject *self, PyObject *arg)
{
    ...
    if (
        ...
        if (PyErr_ExceptionMatches(PyExc_AttributeError) && PyDict_Check(arg)) {
            PyErr_Clear();
            return __object_update_dict__(self, arg);
        }
        return -1;
    }
    Py_DECREF(result);
...
static int
__PyObject_SetState__(PyObject *self, PyObject *arg, PyObject *setter)
{
    PyObject *result = NULL;
    ...
    return __object_setstate__(self, arg);
}
...
static inline int
__object_inplace_concat_or_add__(PyObject *self, PyObject *arg)
{
...
static int
__PyObject_Extend__(PyObject *self, PyObject *arg)
{
    ...
    if (
        ...
        if (PyErr_ExceptionMatches(PyExc_AttributeError)) {
            PyErr_Clear();
            return __object_inplace_concat_or_add__(self, arg);
        }
        return -1;
    }
    Py_DECREF(result);
...
static PyObject *
__PySequence_Fast__(PyObject *obj, Py_ssize_t len, const char *message)
{
    PyObject *result = NULL;
...
static inline int
__object_merge_from_iter__(PyObject *self, PyObject *iter)
{
    PyObject *item = NULL, *fast = NULL;

    while ((item = PyIter_Next(iter))) {
        if (
            !(fast = __PySequence_Fast__(item, 2, "not a sequence")) ||
            PyObject_SetItem(
                self,
...
static inline int
__object_merge__(PyObject *self, PyObject *arg)
{
    ...
    if (iter) {
        res = __object_merge_from_iter__(self, iter);
        Py_DECREF(iter);
    }
    return res;
}
...
static int
__PyObject_Update__(PyObject *self, PyObject *arg)
{
    ...
    if (
        ...
        if (PyErr_ExceptionMatches(PyExc_AttributeError)) {
            PyErr_Clear();
            return __object_merge__(self, arg);
        }
        return -1;
    }
    Py_DECREF(result);
...
static int
__PyCallable_Check__(PyObject *arg, void *addr)
{
...
static int
__PyCallableOrNone_Check__(PyObject *arg, void *addr)
{
    if (arg == Py_None) {
        *(PyObject **)addr = arg;
        return 1;
    }
    return __PyCallable_Check__(arg, addr);
}
...
PyObject *
__instance_new__(PyObject *reduce)
{
    ...
    if (
        PyArg_ParseTuple(
            reduce, "O&O!|OOOO&",
            __PyCallable_Check__, &callable,
            &PyTuple_Type, &args,
            &setstatearg, &extendarg, &updatearg,
            __PyCallableOrNone_Check__, &statesetter
        ) &&
        (self = PyObject_CallObject(callable, args)) &&
        (
            (
                setstatearg != Py_None &&
                __PyObject_SetState__(self, setstatearg, statesetter)
            ) ||
            (extendarg != Py_None && __PyObject_Extend__(self, extendarg)) ||
            (updatearg != Py_None && __PyObject_Update__(self, updatearg))
        )
    ) {
        Py_CLEAR(self);
    }
