...
class _TestCase_(unittest.TestCase):
    ...
    def _test_pack(self, value):
        ...
        self.assertEqual(pack.pack(value), _packed)
        return _packed

    def _test_unpack(self, value):
        return pack.unpack(self._test_pack(value))

    def _test_equal(self, values):
        for value in values:
            self.assertEqual(self._test_unpack(value), value)

    def _test_is(self, values):
        for value in values:
            self.assertIs(self._test_unpack(value), value)
...
class TestInt(_TestCase_, _TestInt_):
    ...
    def test_fixint(self):
        self._test_equal(range(-32, 0))

    def test_int8(self):
        self._test_equal(range(-128, -32))

    def test_int16(self):
        self._test_equal(self._samples(15))

    def test_int32(self):
        self._test_equal(self._samples(31))

    def test_int64(self):
        self._test_equal(self._samples(63))

    def test_overflow(self):
        self.assertRaises(OverflowError, pack.pack, -(1 << 63)-1)
...
class TestUint(_TestCase_, _TestInt_):
    ...
    def test_fixuint(self):
        self._test_equal(range(0, 128))

    def test_uint8(self):
        self._test_equal(range(128, 256))

    def test_uint16(self):
        self._test_equal(self._samples(16))

    def test_uint32(self):
        self._test_equal(self._samples(32))

    def test_uint64(self):
        self._test_equal(self._samples(64))

    def test_overflow(self):
        self.assertRaises(OverflowError, pack.pack, (1 << 64))
...
class TestFloat(_TestCase_):
    ...
    def test_limits(self):
        self._test_equal(self.limits)

    def test_constants(self):
        self._test_equal((-math.inf, math.inf))
        self._test_pack(math.nan)

    def test_float(self):
        for i in range(0, len(self.limits) - 1):
            self._test_equal(self._samples(self.limits[i], self.limits[i + 1]))
...
class TestSeq(_TestSeq_):
    ...
    def test_seq16(self):
        self._test_equal(self._samples(16, k=2))

    def test_seq32(self):
        self._test_equal(self._samples(32, k=1))
...
class TestBytes(_TestCase_, TestSeq):
    ...
    def test_bin8(self):
        self._test_equal(self._samples(8, 0))
...
class TestStr(_TestCase_, TestSeq):
    ...
    def test_fixstr(self):
        self._test_equal((self._i * s for s in range(0, 32)))

    def test_str8(self):
        self._test_equal(self._samples(8, 5))
...
class _TestCont_(object):
    ...
    def test_cont16(self):
        self._test_equal(self._samples(16, 4, k=2))

    def test_cont32(self):
        #self._test_equal(self._samples(32, k=1))
        self._test_equal(self._samples(24, 16, k=1))
...
class TestTuple(_TestCase_, _TestSeq_, _TestCont_):
    ...
    def test_fixarray(self):
        self._test_equal((self._i * s for s in range(0, 16)))
...
class TestSet(_TestCase_, _TestCont_):
    ...
    def _samples(cls, stop, start=-1, k=4):
        if start:
            if start < 0:
                start = (stop >> 1)
            start = (1 << start)
        return (set(range(s)) for s in random.sample(range(start, (1 << stop)), k))

    def test_fixset(self):
        self._test_equal((set(range(s)) for s in range(0, 16)))
...
class TestDict(_TestCase_, _TestCont_):
    ...
    def test_fixmap(self):
        self._test_equal((dict((i, None) for i in range(s))
                            for s in range(0, 16)))
